# -*- coding: utf-8 -*-
"""
Created on Sun Nov 27 22:23:54 2016
...
def plot():
	...
	plt.plot(x_M,y_M,label='Mercury')
	plt.plot(x_V,y_V,label='Venus')
	plt.plot(x_E,y_E,label='Earth')
	plt.plot(x_Ma,y_Ma,label='Mars')
	plt.plot(x_J,y_J,label='Jupiter')
	plt.plot(x_S,y_S,label='Saturn')
	plt.plot(x_U,y_U,label='Uranus')
	plt.plot(x_N,y_N,label='Neptune')
	plt.plot(x_P,y_P,label='Pluto')
	plt.legend(loc = 'upper right',fontsize = 12)
	plt.title('Orbits of planets in Solar System',fontsize=15)
	plt.xlabel('x/AU')
	plt.xlim(-1.8,1.8)
	plt.ylabel('y/AU')
	plt.ylim(-1.8,1.8)
