...
class billiard_innercircle:
    ...
    def plot(self):
        ...
        plt.title('Billiard Trajectory in Outtersqure-Innercircle Stadium',fontsize=16)
    ...
    def phase_space_plot(self):
        ...
        plt.title('Phase Space Diagram($x-v_x$)in Outtersqure-Innercircle Stadium',fontsize=16)								
        plt.xlabel('$x(\mathrm {m})$',fontsize=12)
        plt.ylabel('$v_x(\mathrm {m/s})$',fontsize=12)
        plt.text(1.55,0.7,'$v_{x0}=%.2f\mathrm {m/s}$'%self.vx_0,fontsize=12)	
        plt.text(1.55,0.6,'$v_{y0}=%.2f\mathrm {m/s}$'%self.vy_0,fontsize=12)	
        plt.text(1.55,0.5,'$x_{0}=%.2f\mathrm {m}$'%self.x_0,fontsize=12)
        plt.text(1.55,0.4,'$y_{0}=%.2f\mathrm {m}$'%self.y_0,fontsize=12)									
        plt.scatter(self.x,self.vx, color='r',s=1)
        plt.show()
    
    def phase_plot(self):
        plt.figure(figsize = (8,8))							
        plt.title('Phase Space Section($x-v_x$,$y=0$) in Outtersqure-Innercircle Stadium',fontsize=16)		
        plt.xlabel('$x(\mathrm {m})$',fontsize=12)
        plt.ylabel('$v_x(\mathrm {m/s})$',fontsize=12)								
        record_x = []
        record_vx = []
...
A = billiard_innercircle(0.5,0.5,0.6,0.323,50000,0.1)
A.motion_calculate()
A.plot()
A.phase_space_plot()
A.phase_plot()
